...
import { MAX_POST_IMAGES } from "@/modules/core/posts";
import type { Category } from "@/modules/core/categories";
import { COLOR_CLASSES } from "@/modules/core/categories";
import { MultiImageUpload, type UploadedImage } from "@/modules/uploads";
...
export function EditPostForm({
  ...
  categories,
  canPin,
  canToggleCollab = false,
}: {
  post: Post;
  categories: Category[];
  canPin: boolean;
  /** Owner / moderator only — collaborators can't change the collab switch. */
  ...
  const [images, setImages] = useState<UploadedImage[]>(post.images ?? []);
  const [allowCollab, setAllowCollab] = useState(Boolean(post.allowCollab));
    ...
    startTransition(async () => {
      try {
        await updatePostAction({
          ...
          images,
          allowCollab: canToggleCollab ? allowCollab : undefined,
        });
